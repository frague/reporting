...
WriteFile("temp.tmp", FillTemplate(page, {"##CONSUMERS##": subscribers, "##QUEUES##": "".join(result), "##UPDATED##": datetime.datetime.today().strftime("%b %d, %Y (%H:%M)")}))
GetWiki({"action": "storePage", "space": config["personal_space"], "title": "ActiveMQ queues utilization (autogenerated)", "file": "temp.tmp", "parent": config["parent_page"]})
os.remove("temp.tmp")
